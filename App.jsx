...
export default function ADHDRefocusGame() {
  ...
  const renderResults = () => {
    ...
    if (name) {
      setLeaderboard((prev) => [...prev, { name, score: newScore }]
        .sort((a, b) => b.score - a.score).slice(0, 5));
    }
    return (
      <div>
        <p className="text-xl">You remembered {correct.length} out of {flashHistory.length} correctly!</p>
        <p className="text-lg">Score: {newScore}</p>
        {stage < 10 ? (
          <button onClick={handleNextStage} className="mt-4 px-4 py-2 bg-green-500 text-white rounded">Next Stage</button>
        ) : (
          <div>
            <p className="mt-4 text-xl font-bold">Game Over!</p>
            <button onClick={() => window.location.reload()} className="mt-2 px-4 py-2 bg-purple-600 text-white rounded">Play Again</button>
          </div>
        )}
      </div>
    );
    ...
    <TopographicBackground>
      <div className="p-6 text-center bg-white bg-opacity-80 rounded-xl shadow-xl max-w-xl w-full">
        ...
        <div className="flex justify-between items-center mb-4">
          <p>Stage: {stage}</p>
          <p>Score: {score}</p>
          <button onClick={() => setMuted(!muted)} className="text-sm text-gray-600 underline">{muted ? "Unmute" : "Mute"}</button>
        </div>
        <AnimatePresence>
          {isFlashing ? (
            <ShapeFlasher key={currentShape?.shape + currentShape?.color + Math.random()} shape={currentShape?.shape} color={currentShape?.color} />
          ) : flashHistory.length > 0 ? (
            <div>
              <p>Select the shapes you remember in order:</p>
              <div className="flex justify-center gap-4 mt-4">
                {shapes.map((shape) => (
                  <button key={shape} className="px-4 py-2 bg-blue-500 text-white rounded" onClick={() => handleUserGuess(shape)}>
                    {shape}
                  </button>
                ))}
              </div>
